...
def _predict_vertices(param, roi_bbox, dense, transform=True):
    vertex, vertex_add = reconstruct_vertex(param, dense=dense)
    ...
    if vertex_add is not None:
        sx, sy, ex, ey = roi_bbox
        scale_x = (ex - sx) / 120
        scale_y = (ey - sy) / 120
        vertex_add[0, :] = vertex_add[0, :] * scale_x + sx
        vertex_add[1, :] = vertex_add[1, :] * scale_y + sy

        s = (scale_x + scale_y) / 2
        vertex_add[2, :] *= s

    return vertex, vertex_add
