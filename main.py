...
def main(args):
    ...
    for img_fp in args.files:
        ...
        vertices_lst1 = []  # store multiple face vertices
        ind = 0
        ...
        for rect in rects:
            ...
            # 68 pts
            pts68, _ = predict_68pts(param, roi_box)
            ...
            # 添加 vertices_add无表情 和 vertices_lst2
            if args.dump_ply or args.dump_vertex or args.dump_depth or args.dump_pncc or args.dump_obj:
                vertices_before, vertices_add = predict_dense(param, roi_box)
                vertices_lst1.append(vertices_before)
            if args.dump_ply:
                dump_to_ply(vertices_before, tri, '{}_{}.ply'.format(img_fp.replace(suffix, ''), ind))
                # dump_to_ply(vertices_add, tri, '{}_{}_add.ply'.format(img_fp.replace(suffix, ''), ind))  # 无表情
            if args.dump_vertex:
                dump_vertex(vertices_before, '{}_{}.mat'.format(img_fp.replace(suffix, ''), ind))
                # dump_vertex(vertices_add, '{}_{}_add.mat'.format(img_fp.replace(suffix, ''), ind))       # 无表情
            if args.dump_pts:
            ...
            if args.dump_obj:
                ...
                colors = get_colors(img_ori, vertices_before)
                write_obj_with_colors(wfp, vertices_before, tri, colors)
                print('Dump obj with sampled texture to {}'.format(wfp))

                wfp = '{}_{}_add.obj'.format(img_fp.replace(suffix, ''), ind)
                colors = get_colors(img_ori, vertices_add)
                write_obj_with_colors(wfp, vertices_add, tri, colors)
                print('Dump obj with sampled texture to {}'.format(wfp))
        ...
        if args.dump_depth:
            ...
            # depths_img = get_depths_image(img_ori, vertices_lst1, tri-1)  # python version
            depths_img = cget_depths_image(img_ori, vertices_lst1, tri - 1)  # cython version
            cv2.imwrite(wfp, depths_img)
            print('Dump to {}'.format(wfp))
        if args.dump_pncc:
            wfp = img_fp.replace(suffix, '_pncc.png')
            pncc_feature = cpncc(img_ori, vertices_lst1, tri - 1)  # cython version
